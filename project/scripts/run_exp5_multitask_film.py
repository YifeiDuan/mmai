#!/usr/bin/env python3
"""Exp 5: Multi-task FiLM with Huber loss + cosine LR + AMP + grad clip + wandb logging.

Stacks on top of Exp 3 FiLM (SoTA = test MAE 0.5094 eV) with five independent levers
intended to comfortably beat the baseline:
  1. is_metal auxiliary classification head (BCE, weight 0.3)
  2. Huber regression loss (delta=0.5)
  3. Cosine LR schedule with 10% warmup
  4. Gradient clipping (max norm 1.0)
  5. AMP fp16 mixed precision

Run a single seed:
    python scripts/run_exp5_multitask_film.py --seed 42
Run with smoke test:
    python scripts/run_exp5_multitask_film.py --seed 42 --smoke
"""
...
def parse_args():
    ...
    p.add_argument("--config", default="configs/exp5_multitask_film.yaml")
    p.add_argument("--seed", type=int, default=None,
                   help="Override seed in config; used by SLURM array.")
    p.add_argument("--smoke", action="store_true",
                   help="Run 1 epoch, batch size 4, 8 train samples — for fast end-to-end check.")
    p.add_argument("--no-warm-start", action="store_true",
                   help="Skip loading exp3 FiLM checkpoint.")
    p.add_argument("--align-init", type=str, default=None,
                   help="Path to a CLIPAlignmentModel checkpoint (e.g. results/exp4_alignment/stage1/best_model.pt). "
                        "If set, initialise CGCNN+BERT from the aligned encoders instead of "
                        "exp1/exp2 baselines. FiLM warm-start is automatically disabled.")
    p.add_argument("--no-wandb", action="store_true",
                   help="Disable wandb regardless of config.")
    return p.parse_args()
...
def load_pretrained(model: MultiTaskFiLM, cfg: dict, args, device: torch.device):
    """Load CGCNN + BERT (always) and optionally FiLM warm-start.

    If --align-init was passed, CGCNN+BERT come from a CLIPAlignmentModel
    checkpoint (state-dict keys ``cgcnn.*`` and ``bert.*``) instead of the
    exp1/exp2 baselines. FiLM warm-start is skipped in that case because the
    aligned encoders have a different topology of "good" basins than the
    single-task FiLM ckpt does.
    """
    train_cfg = cfg["train"]

    if args.align_init:
        align_path = Path(args.align_init)
        if not align_path.is_absolute():
            align_path = PROJECT_ROOT / align_path
        if not align_path.exists():
            raise FileNotFoundError(f"--align-init ckpt missing: {align_path}")
        state = torch.load(align_path, map_location=device, weights_only=True)
        cgcnn_state = {k[len("cgcnn."):]: v for k, v in state.items() if k.startswith("cgcnn.")}
        bert_state = {k[len("bert."):]: v for k, v in state.items() if k.startswith("bert.")}
        miss_c, unexp_c = model.cgcnn.load_state_dict(cgcnn_state, strict=False)
        miss_b, unexp_b = model.bert.load_state_dict(bert_state, strict=False)
        print(f"[ckpt] ALIGN-init from {align_path.name}")
        print(f"  CGCNN: missing {len(miss_c)}, unexpected {len(unexp_c)}")
        print(f"  BERT:  missing {len(miss_b)}, unexpected {len(unexp_b)}")
        return  # Skip film warm-start
    ...
    if cgcnn_ckpt.exists():
        ...
        print(f"[ckpt] CGCNN ckpt not found at {cgcnn_ckpt}, training from scratch")
    ...
    if bert_ckpt.exists():
        ...
        print(f"[ckpt] BERT ckpt not found at {bert_ckpt}, training from HF init")

    use_warm = train_cfg.get("use_film_warm_start", True) and not args.no_warm_start
    film_ckpt = PROJECT_ROOT / train_cfg.get("film_ckpt", "")
    if use_warm and film_ckpt.exists():
        state = torch.load(film_ckpt, map_location=device, weights_only=True)
        miss, unexp = model.load_state_dict(state, strict=False)
        bad_missing = [k for k in miss if not k.startswith("metal_head")]
        bad_unexp = list(unexp)
        print(f"[ckpt] FiLM warm-start loaded — metal_head missing (expected): {len([k for k in miss if k.startswith('metal_head')])}, "
              f"other missing: {bad_missing}, unexpected: {bad_unexp}")


def train_one_epoch(model, loader, optimizer, scheduler, scaler, device,
                    grad_clip, alpha_metal, huber_delta, use_amp, log_step_fn=None,
                    epoch_idx: int = 0):
    model.train()
    n_total, l_total, l_reg_total, l_metal_total = 0, 0.0, 0.0, 0.0
    for step_idx, batch in enumerate(loader):
        (atom_fea, nbr_fea, nbr_idx, crys_idx,
         input_ids, attn_mask, target_bg, target_metal) = [b.to(device, non_blocking=True) for b in batch]
...
def main():
    ...
    print(f"[Exp5] seed={seed}, device={device}, amp={use_amp}, smoke={args.smoke}")
    ...
    print(f"[Exp5] Train={len(train_df)}, Val={len(val_df)}, Test={len(test_df)}")
    ...
    load_pretrained(model, cfg, args, device)
    if cfg["train"]["freeze_cgcnn"]:
        model.freeze_cgcnn()

    n_total = sum(p.numel() for p in model.parameters())
    n_train = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[Exp5] params total={n_total:,}, trainable={n_train:,}")
    ...
    print(f"[Exp5] training done in {elapsed:.1f}s, best val MAE={best_val_mae:.4f}")
    ...
    print(f"\n[Exp5] TEST seed={seed}: MAE={test_m['mae']:.4f} RMSE={test_m['rmse']:.4f} "
          f"R2={test_m['r2']:.4f} metal_acc={test_m['metal_acc']:.3f}")

    # Persist
    results = {
        "experiment": "exp5_multitask_film",
        ...
        "config_summary": {
            "lr": cfg["train"]["lr"], "batch_size": bs,
            "epochs_max": epochs, "patience": patience,
            "huber_delta": cfg["loss"]["huber_delta"],
            "alpha_metal": cfg["loss"]["alpha_metal"],
            "warm_start_film": cfg["train"].get("use_film_warm_start", True) and not args.no_warm_start,
            "amp": use_amp, "grad_clip": cfg["train"]["grad_clip_norm"],
        },
    }
