...
Same training recipe as Exp 5 (multi-task is_metal head + Huber regression
loss + cosine LR + AMP + grad clip + multi-seed) but with the FiLM fusion
replaced by token-level cross-attention.
"""
...
def load_pretrained(model: CrossAttnFusion, cfg: dict, device: torch.device):
    train_cfg = cfg["train"]
    ...
    if cgcnn_ckpt.exists():
        ...
        print(f"[ckpt] CGCNN ckpt missing at {cgcnn_ckpt}")
    ...
    if bert_ckpt.exists():
        ...
        print(f"[ckpt] BERT ckpt missing at {bert_ckpt}")
...
def main():
    ...
    print(f"[Exp6] seed={seed}, device={device}, amp={use_amp}, smoke={args.smoke}")
    ...
    print(f"[Exp6] Train={len(train_df)}, Val={len(val_df)}, Test={len(test_df)}")
    ...
    print(f"[Exp6] params total={n_total:,}, trainable={n_train:,}")
    ...
    print(f"[Exp6] training done in {elapsed:.1f}s, best val MAE={best_val_mae:.4f}")
    ...
    print(f"\n[Exp6] TEST seed={seed}: MAE={test_m['mae']:.4f} RMSE={test_m['rmse']:.4f} "
          f"R2={test_m['r2']:.4f} metal_acc={test_m['metal_acc']:.3f}")

    results = {
        "experiment": "exp6_crossattn",
